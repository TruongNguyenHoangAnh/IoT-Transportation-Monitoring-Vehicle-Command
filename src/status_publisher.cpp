...
void publishNodeStatusPing() {
  ...
  for (int node_id = 0; node_id < MAX_VEHICLES; node_id++) {
    ...
    case 0:
      ...
      break;
    case 1:
      vehicleId = "Transport-1";
      break;
    case 2:
      vehicleId = "Transport-2";
      break;
    default:
      vehicleId = String("Node-") + node_id;
      break;
    }
    ...
    // publishNodeStatus(vehicleId, now, wifiRssi, heapFree, packetCountArr,
    //                   rejectedCountArr, online);
  }
}
...
void publishGatewayGpsStatusIfNeeded() {
  ...
  // Publish GPS regardless of fix status (needed for command_vehicle tracking)
  // If no fix, will publish 0,0 with status indicator
  if (!hasGpsFix()) {
    LOG_WARN("[GPS-PUBLISH] No fix attempt (status=%s) lat=%.5f lon=%.5f",
             gpsStatus.c_str(), latitude, longitude);
  } else {
    LOG_INFO("[GPS-PUBLISH] Publishing with FIX lat=%.5f lon=%.5f", latitude,
             longitude);
  }
